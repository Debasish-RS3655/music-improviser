...
Version = '5.6  for Python3'
VersionDate = '20190512'

# 20190512 5.6 moved import fcntl and termios to stat() to accomodate windows
...
def stat(score=None, params=None):
    ...
    if params and len(params)>0 and params[0] == '-freq':
        ...
        try:
            import fcntl   # 5.6 moved here to allow for missing fcntl
            import termios # 5.6 moved here to allow for missing termios
...
Possible_Effects = ['compand','echo','fade','key','mixer','pad','pan','pitch',
 'quantise','quantize','repeat','silence', 'stat','stats','tempo','trim','vol']
global_options = []
input_files    = []
output_file    = [[], '']
effects        = []
...
for effect in effects:
    ...
    elif effect[0] == 'stat' or effect[0] == 'stats':
